...
export const Final = () => {
  ...
  const secretCode = import.meta.env.SECRET_CODE || "nvimqmihim";
  const key = import.meta.env.KEY || "vue";
  const decryptedWord = import.meta.env.DECRYPTED_WORD || "sberwinner";
  ...
  return (
    <div className="rebus final">
      ...
      {decryptedWord === value.toLowerCase() && (
        <div>Финальный код: {answer}. Поздравляем!</div>
      )}
    </div>
  );
};
